...
- モバイルメニュー（`nav` > `.mobile-menu__inner`）の内容が全ページで同一か
  - `.mobile-menu__close`, `.mobile-menu__phone`, `.mobile-menu__cta`, `.mobile-menu__footer` が全ページに存在するか
  - オーバーレイスタイル: `rgba(11,22,40,0.85)` + `backdrop-filter: blur(4px)` が統一されているか
...
- `.scroll-progress`（スクロール進捗バー）が全ページに存在するか
...
### 5. コンテンツ整合性
- 電話番号 `03-4363-7710` が全ページで統一されているか
- 受付時間「年中無休 / 13:00〜20:00」が全ページで統一されているか

...
└── .claude/
    ...
    ├── LEARNINGS.md    # 自己改善ログ（/retro が蓄積）
    └── commands/       # Claude Code スキル定義（11スキル）
```
...
| `/retro` | 振り返り→ハーネス自己改善（学びを分類し CLAUDE.md/コマンド/LEARNINGS へ反映） |

## §10 自己改善ループ（Self-Improvement Loop）

このハーネスは**固定の手順書ではなく、運用しながら自分を直していく仕組み**として設計する。
手順をガチガチに縛らず、不変条件の中で Claude が最善手を選び、ズレに気づいたら手順そのものを更新する。

### 不変条件（hard）と 既定値（soft）の区別

すべてのルールを同じ重さで守ろうとすると動きが鈍る。守るべき強度を2段に分ける:

- **不変条件（hard・絶対に守る — 破ると壊れる）**
  - §2-1 HTML Valid（DOCTYPE / lang / 閉じタグ）
  - §2-4 4ページ統一（ヘッダー/フッター/ナビ/フローティングCTA）
  - §2-7 No Guessing（読んで確認してから変更）
  - §6 アクセシビリティの WCAG 2.1 AA 必須項目（alt / フォーカス / コントラスト）
- **既定値（soft・効率のための既定。理由があれば外してよい）**
  - §1 ファイル読込2ファイル制限 / コミット単位「1機能=1コミット」
  - §5 Tailwind クラスの並び順 / コンポーネント命名の細部
  - 各コマンドの手順の順番（実態に合わなければ並べ替え・スキップ可）

soft を外したときは、なぜ外したかを `/retro` で記録する。常態的に外すなら、それは soft の定義が古い合図 → ルール側を直す。

### ループの回し方

```
作業 → /retro → 学びを分類（invariant / gotcha / command-fix / fact / loosen）
     → ハーネスへ最小 diff で反映 → .claude/LEARNINGS.md に蓄積
```

### いつ回すか
- 1機能を完了したとき（`/compact` の前後）
- コマンドの手順が実態とズレた／足りないと気づいたとき
- 新しい罠を踏んだとき（同じ失敗を二度しないため）
- CLAUDE.md の事実（行数・店舗数・init 関数一覧）が古くなったと気づいたとき

### 蓄積先
- `.claude/LEARNINGS.md` — 日付つきの学びログ。次回 `/retro` で CLAUDE.md / コマンドへ昇格する候補。
- ハーネスを**太らせない**こと。重複する学びは既存記述を更新し、古い手順・効かない制約は削るか soft に格下げする。
